...
<body class="w-full min-h-screen flex justify-center items-center">
  ...
  <header class="px-20 h-[100px] flex fixed top-0 w-full justify-between items-center">
    <a href="./"><span class="text-2xl text-[#6A75F1] mr-2">Quizzaps</span> by Kelompok 4</a>
  </header>

  <main class="max-w-md w-full text-center flex-col justify-center mb-4 px-8 py-20 bg-white/5 backdrop-blur-sm shadow-xl rounded-3xl">
    <form action="./quiz.php" method="post" class="w-full flex flex-col items-start gap-4">

      <span class="label-text ml-1">Nama:</span>
      <input type="text" class="input w-full" placeholder="Nama" name="nama">
      
      <span class="label-text ml-1">NPM:</span>
      <input type="number" class="input w-full" placeholder="NPM" name="npm">
      
      <span class="label-text ml-1">Kelas:</span>
      <input type="text" class="input w-full" placeholder="Kelas" name="kelas">

      <div class="w-full mt-4">
        <input type="submit" value="Start" class="btn btn-primary mx-auto font-bold tracking-wide px-8">
      </div>
    </form>
  </main>
</body>
